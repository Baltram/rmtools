...
namespace
{
    ...
    void WriteMesh( mCMesh const & a_meshSource, mCIOStreamBinary & a_streamDest, mCAseWriter::SOptions const & a_Options )
    {
        ...
        if ( a_meshSource.HasVertexColors() && !a_Options.m_bDropVertexColors )
        {
        ...
        if ( !a_Options.m_bDropVertexNormals )
        {
            mCMesh meshClone;
    ...
    void WriteNode( mCScene const & a_sceneSource, MIUInt a_uNodeIndex, mCIOStreamBinary & a_streamDest, mCAseWriter::SOptions const & a_Options )
    {
        ...
        if ( pParent )
            a_streamDest.Write( GetTokenLine( "NODE_PARENT", "\"" + pParent->GetName() + "\"" ) );
...
{
    a_streamDest.Write( GetTokenLine( "3DSMAX_ASCIIEXPORT", "200" ) );
    ...
    MIUInt const uMaterialCount = a_sceneSource.GetNumMaterials();
    ...
    for ( MIUInt u = 0; u != uMaterialCount; ++u )
    {
        ...
        WriteMaterial( a_sceneSource.GetMaterialAt( u ), a_streamDest );
        a_streamDest.Write( EndBlock() );
    }
    a_streamDest.Write( EndBlock() );
    for ( MIUInt u = 0, ue = a_sceneSource.GetNumNodes(); u != ue; ++u )
    {
        ...
        WriteNode( a_sceneSource, u, a_streamDest, a_Options );
        a_streamDest.Write( EndBlock() );
    }
    return mEResult_Ok;
}
// ASE2XMSH.cpp : Definiert den Einstiegspunkt für die Anwendung.
//
                     ...
                     int       nCmdShow)
{
	...
	//start
	int matcount = 0;
	int objects[2 * 1000]; //{pos, matid} * 1000
	char *matname;
	char mnbuffer[500];

	if (ase.searchbytes("*MATERIAL_COUNT", 15) > (-1))
	{
		ase.linein(filter, 2);
		matcount = ase.lint(1);
	}
	twodim mats(20000, (matcount + 1));
	for (int i = 0;i < matcount;i++)
	{
		ase.searchbytes("*MATERIAL_NAME", 14);
		ase.linein(filter, 2);
		matname = ase.lascii(1);
		memcpy(mnbuffer, matname, (strlen(matname) + 1));
		matname = mnbuffer;
		ase.linein(filter, 2);
		if ((ase.lascii(1))[1] == 'M') //*MATERIAL_CLASS "Multi/Sub-Object"
		{
			ase.searchbytes("*NUMSUBMTLS", 11);
			ase.linein(filter, 2);
			int numsubmtls = ase.lint(1);
			for (int j = 0;j < numsubmtls;j++)
			{
				ase.searchbytes("*MATERIAL_NAME", 14);
				ase.linein(filter, 2);
				matname = ase.lascii(1);
				mats.addstring(i, (strlen(matname) - 2), &matname[1]);
			}
		}
		else //*MATERIAL_CLASS "Standard"
		{
			mats.addstring(i, (strlen(matname) - 2), &matname[1]);
		}
	}
	if (matcount == 0)
	{
		mats.addstring(0, 6, "(null)");
		matcount = 1;
	}
	
	objects[0] = 0;
	while (ase.searchbytes("*MESH_NUMVERTEX", 15) > objects[0])
	{
		objects[objcount * 2] = ase.getreadpos();
		ase.linein("", 0);
		ase.linein(filter, 2);
		allindcount += (ase.lint(1) * 3);
		ase.searchbytes("*PROP_RECVSHADOW", 16);
		ase.linein("", 0); //skip one line
		ase.linein(filter, 2);
		if (ase.numlineparts() > 1) // "*MATERIAL_REF x", not "}"
		{
			objects[objcount * 2 + 1] = ase.lint(1);
			if (objects[objcount * 2 + 1] >= matcount)
			{
				objects[objcount * 2 + 1] = 0;
			}
		}
		else
		{
	...
	{
		newobjcount += mats.getcount(objects[2 * i + 1]);
	}

	/*
	for (int i = 0;i < matcount;i++)
	{
		for (int j = 0;j < mats.getcount(i);j++)
		{
			output::msg(mats.getstring(i, j),"e");
		}
	}
	*/
	...
	for (int i = 0;i < objcount;i++)
	{
		int matnum = objects[(2 * i) + 1];
		int c = mats.getcount(matnum);
		for (int j = 0;j < c;j++) 
		{
			objdummys od;
			...
			matname = mats.getstring(matnum, j);
	...
	for (int i = 0;i < objcount;i++)
	{
		...
		mesh newmesh(&ase, objects[2 * i], c, angle);
	...
	LoadString(hInstance, IDC_ASE2XMSH, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_ASE2XMSH));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ASE2XMSH));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_ASE2XMSH);
